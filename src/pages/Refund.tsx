        ...
        <div className="prose prose-sm dark:prose-invert max-w-none space-y-8">
          ...
          <section className="space-y-4">
            ...
            <div className="pl-4 border-l-2 border-muted space-y-4">
              <h3 className="font-semibold text-lg">4.1 {language === 'ar' ? 'حدود الاستخدام الجوهري (خلال أول 7 أيام)' : 'Material-use limits (within the first 7 days)'}</h3>

              <p>{language === 'ar' ? 'استرداد الأموال غير متاح إذا قمت بأي مما يلي:' : 'Refunds are not available if you have done any of the following:'}</p>

              <div className="space-y-4 mt-4">
                <div>
                  <h4 className="font-semibold">A) {language === 'ar' ? 'تحليل الاختبارات السابقة / تقرير العائد' : 'Past Exam Analysis / Yield Report'}</h4>
                  <ul className="list-disc list-inside mt-1 ml-4">
                    <li>{language === 'ar' ? 'أكملت أي "تحليل اختبار سابق" ينتج عنه تعيين للمواضيع و/أو تقرير العائد.' : 'Completed any “Past Exam Analysis” that produces topic mapping and/or a Yield Report.'}</li>
                  </ul>
                </div>

                <div>
                  <h4 className="font-semibold">B) {language === 'ar' ? 'استخراج المنهج بالذكاء الاصطناعي' : 'AI Syllabus Extraction'}</h4>
                  <ul className="list-disc list-inside mt-1 ml-4">
                    <li>{language === 'ar' ? 'أجريت أكثر من 2 عملية استخراج للمنهج (PDF/صورة/OCR).' : 'Performed more than 2 AI syllabus extractions (PDF/image/OCR).'}</li>
                  </ul>
                </div>

                <div>
                  <h4 className="font-semibold">C) {language === 'ar' ? 'إنشاء الاختبارات بالذكاء الاصطناعي' : 'AI Quiz Generation'}</h4>
                  <ul className="list-disc list-inside mt-1 ml-4">
                    <li>{language === 'ar' ? 'أنشأت أكثر من 25 اختباراً (أو مجموعات اختبارات مكافئة).' : 'Generated more than 25 AI quizzes (or equivalent quiz sets).'}</li>
                  </ul>
                </div>

                <div>
                  <h4 className="font-semibold">D) {language === 'ar' ? 'تحليل عميق للمواضيع ("تحليل الموضوع")' : 'Topic Deep Dives (“Analyze Topic”)'}</h4>
                  <ul className="list-disc list-inside mt-1 ml-4">
                    <li>{language === 'ar' ? 'أجريت أكثر من 10 تحليلات عميقة للمواضيع.' : 'Performed more than 10 topic deep dives.'}</li>
                  </ul>
                </div>

                <div>
                  <h4 className="font-semibold">E) {language === 'ar' ? 'الخطة الذكية + مزامنة التقويم' : 'Smart Plan + Calendar Sync'}</h4>
                  <ul className="list-disc list-inside mt-1 ml-4">
                    <li>{language === 'ar' ? 'زامنت أكثر من 14 جلسة دراسية/حدثاً مع تقويم Google (أو أنشأت أكثر من 14 جلسة مجدولة داخل التطبيق).' : 'Synced more than 14 study sessions/events to Google Calendar (or created more than 14 scheduled sessions in-app if you mirror these).'}</li>
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside mt-1 ml-4">
                    <li>{language === 'ar' ? 'أنشأت أكثر من 2 مواد Pro (فوق حد الباقة المجانية).' : 'Created more than 2 Pro courses (beyond the free tier course limit).'}</li>
                  </ul>
                </div>

                <div>
                  <h4 className="font-semibold">G) {language === 'ar' ? 'التصدير / التنزيل (إذا كان متاحاً)' : 'Exports / Downloads (if applicable)'}</h4>
                  <ul className="list-disc list-inside mt-1 ml-4">
                    <li>{language === 'ar' ? 'قمت بتصدير أو تنزيل أي تقرير/خطة مميزة (مثل تصدير تقرير العائد، تصدير PDF).' : 'Exported or downloaded any premium report/plan (e.g., Yield Report export, PDF export).'}</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4">
              <h3 className="font-semibold text-lg mb-2">4.2 {language === 'ar' ? 'لماذا توجد هذه الحدود' : 'Why these limits exist'}</h3>
              <p>
                {language === 'ar'
          ...
          <section className="space-y-4">
            ...
            <p>{language === 'ar' ? 'سيتم رفض طلبات الاسترداد إذا انطبق أي مما يلي:' : 'Refunds will be denied if any of the following applies:'}</p>
          ...
          <section className="space-y-4">
            ...
            <p>
              {language === 'ar' ? 'أرسل بريداً إلكترونياً إلى support@studybuddy.app مع:' : 'Email support@studybuddy.app with:'}
            </p>
            <ul className="list-disc list-inside space-y-2">
              <li>{language === 'ar' ? 'عنوان البريد الإلكتروني لحساب StudyBuddy الخاص بك' : 'The email address on your StudyBuddy account'}</li>
