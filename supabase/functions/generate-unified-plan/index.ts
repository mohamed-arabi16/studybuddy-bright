...
serve(async (req) => {
  ...
  try {
    ...
    // ============= P2: CONSUME CREDITS BEFORE PLAN GENERATION =============
    const creditResult = await consumeCredits(supabase, user.id, 'generate_plan');
    
    if (!creditResult.success) {
      console.log(`Insufficient credits for user ${user.id}: have ${creditResult.balance}, need ${creditResult.required}`);
    ...
    console.log(`Credits consumed for plan generation: charged ${creditResult.credits_charged}, balance ${creditResult.balance}`);
